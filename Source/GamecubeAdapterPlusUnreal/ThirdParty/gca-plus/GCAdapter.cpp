...
namespace gca {
	...
	void CheckRumble(bool* tRumble, int arraySize)
	{
        ...
        int payload_size = 0;
        int code = libusb_interrupt_transfer(device_handle, endpoint_out, data, sizeof(data), &payload_size, 16);
	}
	...
	ControllerStatus GetGamepadStatus(uint8_t * _controller_payload, int i) {
		...
		if(!metadata[i].connected_on_prev_poll)
		{
			...
			metadata[i].init_primary_x   = _controller_payload[4 + i * 9];// - 128;
			metadata[i].init_primary_y   = _controller_payload[5 + i * 9];// - 128;
			metadata[i].init_secondary_x = _controller_payload[6 + i * 9];// - 128;
			metadata[i].init_secondary_y = _controller_payload[7 + i * 9];// - 128;
			metadata[i].init_trigger_l   = _controller_payload[8 + i * 9];
			metadata[i].init_trigger_r   = _controller_payload[9 + i * 9];

		}

		status.connected   = GetNthBit(_controller_payload[1 + i * 9], 5) != 0; // occurs in payload bytes: 1, 10, 19, 28

		status.button_a     = GetNthBit(_controller_payload[2 + i * 9], 1) != 0;
		status.button_b     = GetNthBit(_controller_payload[2 + i * 9], 2) != 0;
		status.button_x     = GetNthBit(_controller_payload[2 + i * 9], 3) != 0;
		status.button_y     = GetNthBit(_controller_payload[2 + i * 9], 4) != 0;

		status.pad_left     = GetNthBit(_controller_payload[2 + i * 9], 5) != 0;
		status.pad_right    = GetNthBit(_controller_payload[2 + i * 9], 6) != 0;
		status.padDown     = GetNthBit(_controller_payload[2 + i * 9], 7) != 0;
		status.padUp       = GetNthBit(_controller_payload[2 + i * 9], 8) != 0;

		status.button_start = GetNthBit(_controller_payload[3 + i * 9], 1) != 0;
		status.button_z     = GetNthBit(_controller_payload[3 + i * 9], 2) != 0;
		status.button_r     = GetNthBit(_controller_payload[3 + i * 9], 3) != 0;
		status.button_l     = GetNthBit(_controller_payload[3 + i * 9], 4) != 0;

		// TODO: memcopy these? memcopy all?
		// TODO: add back bias correction & controller_analog_bias struct?

		status.primary_x   = _controller_payload[4 + i * 9] - metadata[i].init_primary_x;
		status.primary_y   = _controller_payload[5 + i * 9] - metadata[i].init_primary_y;

		status.secondary_x = _controller_payload[6 + i * 9] - metadata[i].init_secondary_x;
		status.secondary_y = _controller_payload[7 + i * 9] - metadata[i].init_secondary_y;

		status.trigger_l   = _controller_payload[8 + i * 9] - metadata[i].init_trigger_l;
		status.trigger_r   = _controller_payload[9 + i * 9] - metadata[i].init_trigger_r;
